...
const fiveStrings = strings.filter((str) => str.length >= 5);
...
const alphabetsJoint = alphabets.reduce((acc, current) => acc + current, '');
console.log(alphabetsJoint);
...
const positiveNumbers = positive.every((num) => num > 0);
...
cars.forEach((car, index) => {
  ...
  console.log(`${index + 1}. "${name} ${color[0]} ${capacity}"`);
});
...
console.log(`${person.name} travels by plane called ${person.vehicles.plane}`);

person.email = "ajaykumar@gmail.com";

const belongings = Object.keys(person);
